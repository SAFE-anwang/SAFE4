...
var recvHeightBlocks = make(map[uint64]types.Blocks)

func (s *Spos) CheckRewardTransaction(block *types.Block, receipts types.Receipts) error {
	if s.isTopSuperNode(block.ParentHash()) {
		height := block.NumberU64()
		// check multiple blocks generated by same supernode
		flag := false
		for _, b := range recvHeightBlocks[height] {
			if b.Hash() == block.Hash() || b.Header().Coinbase != block.Coinbase() {
				continue
			}
			flag = true
			log.Info("Find mulitple blocks generated by same miner", "miner", block.Coinbase(), "height", height, "old-block", b.Hash().Hex(), "new-block", block.Hash().Hex())
			if info, err := s.GetSuperNodeInfo(block.Coinbase(), block.ParentHash()); err == nil {
				var ids []*big.Int
				ids = append(ids, info.Id)
				var states []*big.Int
				states = append(states, big.NewInt(3)) // StateDisable
				hash, err := contract_api.UploadSuperNodeStates(s.ctx, s.blockChainAPI, s.transactionPoolAPI, s.signer, ids, states)
				log.Info("Upload supernode state", "caller", s.signer, "ids", ids, "states", states, "hash", hash.Hex(), "error", err)
			}
			break
		}
		if !flag {
			recvHeightBlocks[height] = append(recvHeightBlocks[height], block)
			for k, _ := range recvHeightBlocks {
				if k < height - 120 {
					delete(recvHeightBlocks, k)
				}
			}
		}
	}
...
func (s *Spos) isTopSuperNode(hash common.Hash) bool {
	tops, err := s.GetTops(hash)
	if err != nil {
		return false
	}
	for _, snAddr := range tops {
		if snAddr != s.signer {
			continue
		}
		info, _ := s.GetSuperNodeInfo(s.signer, hash)
